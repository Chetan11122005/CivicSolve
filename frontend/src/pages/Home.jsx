  ...
  return (
    <div className="min-h-screen flex flex-col bg-white text-gray-900">
      ...
      <nav className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
        <div className="text-2xl font-extrabold text-blue-600 tracking-tight">CivicSolve</div>
        <div className="flex gap-4">
          <Link to="/auth" className="text-gray-600 hover:text-gray-900 font-medium px-4 py-2">Log In</Link>
          <Link to="/auth" className="bg-blue-600 text-white px-4 py-2 rounded-md font-medium hover:bg-blue-700">Sign Up</Link>
        </div>
      </nav>
      ...
      <main className="flex-grow">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Link to="/post-challenge" className="inline-flex justify-center items-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 md:text-lg">
              Post a Challenge
            </Link>
            <Link to="/discover" className="inline-flex justify-center items-center px-8 py-3 border border-gray-300 text-base font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 md:text-lg">
              Explore Challenges
            </Link>
          </div>
        </div>
        ...
        <div className="py-20 bg-gray-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-16">
              <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight sm:text-4xl">Success Stories</h2>
              <p className="mt-4 max-w-2xl text-xl text-gray-500 mx-auto">Real-world impact created by collaborative teams across the nation.</p>
            </div>

            {loading ? (
              <div className="text-center text-gray-500">Loading success stories...</div>
            ) : featuredChallenges.length === 0 ? (
              <div className="text-center text-gray-500 bg-white p-10 rounded-lg border border-gray-200">
                Check back soon for featured success stories!
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {featuredChallenges.map((challenge) => {
                  // Find the verified solution to display the summary
                  ...
                  return (
                    <div key={challenge.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col hover:shadow-md transition-all">
                      {challenge.image_url ? (
                      ...
                      <div className="p-6 flex-1 flex flex-col">
                        ...
                        <p className="text-gray-600 text-sm mb-4 flex-1 line-clamp-3">
                          <span className="font-semibold text-gray-900">The Solution: </span>
                          {summaryText}
                        </p>
                        <Link to={`/challenge/${challenge.id}`} className="inline-flex items-center text-blue-600 font-medium hover:text-blue-500">
